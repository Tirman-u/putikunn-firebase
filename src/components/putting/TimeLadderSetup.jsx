  ...
  return (
    <div className="max-w-md mx-auto px-4 pt-8">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-slate-800 mb-2 dark:text-slate-100">Aja väljakutse</h1>
        <p className="text-slate-600 dark:text-slate-300">Solo treening</p>
      </div>
      ...
      <div className="space-y-6 bg-white rounded-2xl p-6 shadow-sm dark:bg-black dark:border dark:border-white/10 dark:text-slate-100">
        <div>
            ...
            Mängu nimi (valikuline)
          </label>
          <Input
            value={gameName}
            onChange={(e) => setGameName(e.target.value)}
            placeholder="Nt. Aja väljakutse"
            className="text-base"
          />
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {DISC_OPTIONS.map((count) => (
              ...
              >
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400">ketast</div>
              </button>
            ))}
          </div>
        </div>
