...
export default function DuelJoin() {
  ...
  const handleJoin = async () => {
    try {
      ...
      const q = query(collection(db, 'duel_games'), where('pin', '==', pin));
      ...
      let nextGame = { ...game, state: nextState };
      ...
      await updateDoc(doc(db, 'duel_games', game.id), nextGame);
      setGameId(game.id);
      ...
      <div className="max-w-lg mx-auto px-4 pb-10">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 space-y-4">
          <div>
            ...
            <Input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Sisesta nimi" className="h-12 rounded-xl border-slate-200" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">PIN</label>
            <Input value={pin} onChange={(e) => setPin(e.target.value)} placeholder="4-kohaline PIN" className="h-12 rounded-xl border-slate-200 text-center text-lg tracking-widest" />
          </div>
          <Button className="w-full h-12 rounded-2xl" onClick={handleJoin} disabled={joining}>{joining ? 'Liitun...' : 'Liitu mänguga'}</Button>
        </div>
      </div>
    </div>
  );
